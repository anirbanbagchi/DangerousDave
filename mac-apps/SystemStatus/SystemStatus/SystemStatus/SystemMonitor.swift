...
//
//  Polling loops:
//   • usageTimer   (refreshInterval, main thread)  — CPU, memory, network, disk, battery
...
final class SystemMonitor {
    ...
    // which doesn't trigger re-renders inside the MenuBarExtra label closure.
    ...
    private func refreshUsage() {
        ...
        // Update sparkline history (capped at 60 points)
        ...
        // Normalise network rates to 0-100 relative to a 100 MB/s ceiling
...
private extension Double {
    /// Returns self if > 0, else nil (avoids treating unset UserDefaults keys as 0).
